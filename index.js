...
async function sendSol(fromKeypair, toPublicKey, amount) {
  ...
  //console.log('\x1b[34m%s\x1b[0m', 'Transaction confirmed with signature:', signature)  // Blue color for success message
}
...
;(async () => {
  ...
  console.log('Generated 1000 random addresses:', randomAddresses)
  ...
  for (const address of randomAddresses) {
    ...
    try {
      ...
      console.log('\x1b[32m%s\x1b[0m', `Successfully sent ${amountToSend} SOL to ${address}`)  // Green color for success message
      successCount++
    } catch (error) {
      console.error('\x1b[33m%s\x1b[0m', `Failed to send SOL to ${address}`)  // Yellow color for error message
      failureCount++
    }
  ...
  console.log('\x1b[32m%s\x1b[0m', `TRANSACTION COMPLETED: ${successCount}`)  // Green color for success count
  console.log('\x1b[31m%s\x1b[0m', `TRANSACTION FAILED: ${failureCount}`)  // Red color for failure count
})()
